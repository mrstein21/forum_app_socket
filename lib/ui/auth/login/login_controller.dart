...
class LoginController extends GetxController{
  ...
  void callLoginService(){
    ...
    AuthRepository().loginService(email: email,password: password).then((value){
      progressDialog.hide().whenComplete(()async{
        var decode_response=json.decode(value);
        if(decode_response["success"]==true){
          userCredential={
            "id":decode_response["data"]["payload"]["id"].toString(),
            "name":decode_response["data"]["payload"]["name"],
            "email":decode_response["data"]["payload"]["email"],
            "token":decode_response["data"]["token"]
          };
          await setup();
